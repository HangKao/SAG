from visualizer import get_local # install the visualizer before using
get_local.activate()
...
class LoRA_Sam(pl.LightningModule):
    ...
    def forward(
          ...
          (list(dict)): A list over input images, where each element is
            as dictionary with the following keys.
              ...
              'low_res_logits': (torch.Tensor) Low resolution logits with
                ...
                to subsequent iterations of prediction.
        """
        imgs = batched_input['image']
        pt = batched_input['pt']
        point_labels = batched_input['p_label']
        bbox_input = batched_input['bbox']
        lowmask_input = batched_input['low_mask']
        

        imgs = torch.as_tensor(imgs, dtype=torch.float, device=self.device)

        if (pt is not None) or (point_labels is not None):
            pt = torch.as_tensor(pt, dtype=torch.float, device=self.device)
            point_labels = torch.as_tensor(point_labels, dtype=torch.float, device=self.device)
            point_coords = pt
            coords_torch = torch.as_tensor(point_coords, dtype=torch.float, device=self.device)
            labels_torch = torch.as_tensor(point_labels, dtype=torch.int, device=self.device)
            pt = (coords_torch, labels_torch)
        ...
        if bbox_input is not None:
            bbox_torch = torch.as_tensor(bbox_input, dtype=torch.float, device=self.device)
        else:
            bbox_torch = None

        if lowmask_input is not None:
            lowmask_torch = torch.as_tensor(lowmask_input, dtype=torch.float, device=self.device)
        else:
            lowmask_torch = None
...
if __name__ == "__main__":
    ...
    model = sam_model_registry["vit_b"]().to(device)
    ...
    baseRt = 8
    metaPath = "./tb_logs/Your_Logs/SAG/"
    ckptName = metaPath+"checkpoints/epoch=%d-step=%d.ckpt"%(int(baseEp+(baseRt-1)*10),int(baseIt*baseRt))
    ...
    lora_sam = LoRA_Sam.load_from_checkpoint(**ckptDict,map_location='cpu')
    lora_sam = lora_sam.eval()


    prefix = "/Your_Path/newSamDz/"
    seis_train_dataset = geoDataset(prefix,"./testXAI/dzValid.txt")
    print(len(seis_train_dataset))
    # sz = seis_train_dataset[50]
    sz = seis_train_dataset[75]
    # sz = seis_train_dataset[150]
    ...
    numPts = 1
    sz['pt'] = sz['pt'][:,:numPts,:]
    sz['p_label'] = sz['p_label'][:,:numPts]


    out = lora_sam.forward(sz)
    
    cache = get_local.cache

    print(cache.keys())

    allProj = cache['_LoRA_output.forward']
    allQKV = cache['_LoRA_qkv.forward']

    saveDict = {}

    for idi,proj in enumerate(allProj):
        projArr = []
        for idj,at in enumerate(proj):
            projArr.append(at)
        
        saveDict['proj%02d'%idi]=projArr

    for idi,qkv in enumerate(allQKV):
        qkvArr = []
        for idj,at in enumerate(qkv):
            qkvArr.append(at)
        
        saveDict['qkv%02d'%idi]=qkvArr

    saveDict['sz'] = sz

    print(saveDict.keys())
    np.savez("./testXAI/out/XAI%dGeobody.npz"%int(baseEp+(baseRt-1)*10),**saveDict)
